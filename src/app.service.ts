...
export class AppService {
  ...
  public getLatestNews() {
    return [
      ...
      },
      {
        id: 3,
        title:
          'Komiko视频内视频功能样式上线！AI一键生动叙述大片，满足创作者从头到脚的创作需求',
        date: '6/4/2025',
      },
      {
        id: 4,
        title:
          'Expensya创始人"春哥"重磅创业，AI测试平台Thunder Code获900万美元种子轮',
        date: '6/4/2025',
      },
      {
        id: 5,
  ...
  getToolsPageData(
    ...
    let tools = this.getHotTools().map((tool) => ({
      ...tool,
      rating: Math.random() * 2 + 3,
      features: ['免费试用', 'API接口', '多语言'].slice(
        0,
        Math.floor(Math.random() * 3) + 1,
      ),
    })); // Add mock rating and features

    // TODO: Implement actual filtering by category
    if (category && category !== '全部' && category !== 'all') {
      // 'all' is from placeholderCategories in ToolsPage.tsx
    ...
    const trendingTools = this.getHotTools()
      .slice(0, 3)
      .map((t) => ({
        id: String(t.id),
        name: t.name,
    ...
    const sidebarNews = this.getLatestNews()
      .slice(0, 3)
      .map((n) => ({ id: String(n.id), title: n.title, time: n.date }));
  ...
  getProjectsPageData(
    ...
    const allMockProjects = [
      {
        ...
        avatarLetter: 'M',
        name: 'MLLMCelltype',
        description: '机器学习细胞类型分析工具，提供高精度的细胞分类和识别功能',
      ...
      },
      {
        ...
        avatarLetter: 'M',
        name: 'Mcp Containers',
        description: '容器化MCP部署解决方案，简化AI模型的部署和管理流程',
      ...
      },
      {
        ...
        avatarLetter: 'S',
        name: 'Scira Mcp Chat',
        description: '基于MCP协议的智能聊天系统，支持多模态对话交互',
        tags: ['TypeScript', 'Chat', 'MCP'],
        stars: 41,
        growth: '+41',
        forks: 6,
        categoryId: 'all',
      ...
      },
      {
        ...
        avatarLetter: 'B',
        name: 'Bert VITS2 MNN',
        description: '基于BERT的语音合成模型，支持多种语言和音色',
        tags: ['Python', 'TTS', 'BERT'],
        stars: 15,
        growth: '+15',
        forks: 3,
        categoryId: 'tts',
      ...
      },
      {
        ...
        avatarLetter: 'E',
        name: 'EShopLite',
        description: '轻量级电商解决方案，集成Deepseek AI推荐系统',
        tags: ['React', 'E-commerce', 'AI'],
        stars: 5,
        growth: '+5',
        forks: 1,
        categoryId: 'deepseek',
        categoryName: 'Deepseek 相关项目',
      ...
      },
      {
        id: 'p6',
        avatarLetter: 'A',
        name: 'Another TTS Project',
        description: 'Another great TTS tool',
        tags: ['Audio', 'TTS'],
        stars: 22,
        growth: '+2',
        forks: 4,
      ...
      },
      {
        ...
        description: 'Analyze data with DeepSeek tech',
    ...
    if (time && time !== 'all') {
      ...
      if (time === 'today' || time === '今日') {
        // '今日' for consistency with UI
        filteredProjects = filteredProjects.filter(
    ...
    if (sort === 'stars') {
      ...
      filteredProjects.sort(
        (a, b) =>
          parseInt(b.growth.substring(1)) - parseInt(a.growth.substring(1)),
      );
    }
    ...
    const projectCategories = [
      {
        ...
        name: '热门项目',
        icon: '🏆',
        count: allMockProjects.length,
      }, // Count might need to be dynamic based on actual data
      {
        id: 'tts',
        name: 'TTS',
        icon: '🎵',
        count: allMockProjects.filter((p) => p.categoryId === 'tts').length,
      },
      {
        id: 'deepseek',
        name: 'Deepseek',
        icon: '🧠',
        count: allMockProjects.filter((p) => p.categoryId === 'deepseek')
          .length,
      },
      {
        id: 'stt',
        ...
        icon: '🎤',
        count:
          allMockProjects.filter((p) => p.categoryId === 'stt').length || 0,
      }, // Example
    ...
    const stats = [
      ...
      {
        id: 'ps2',
        number: `${allMockProjects.reduce((sum, p) => sum + p.stars, 0) / 1000}K+`,
        label: 'GitHub Stars',
      },
      { id: 'ps3', number: '120+', label: '活跃项目' }, // Placeholder
      {
        id: 'ps4',
        number: `${projectCategories.filter((pc) => pc.id !== 'all').length}+`,
        label: '技术分类',
      },
    ];
  ...
  getMcpLandingPageData() {
    ...
    const allServices = this.getHotTools().map((tool) => ({
      // Assuming HotTool structure is similar enough to ServiceData
      id: tool.id,
    ...
    const tutorials = this.getFeaturedContent()
      ...
      .map((item) => ({
        ...
        publishDate: new Date(Date.now() - Math.random() * 10000000000)
          .toISOString()
          .split('T')[0], // Mock date
    ...
    const faqItems = [
      {
        question: 'MCP服务如何收费？',
        answer:
          '大部分MCP服务提供按需付费模式，部分基础服务可能提供免费套餐。详情请查看具体服务的定价页面。HTML <b>加粗</b> 内容也可以。',
      },
      {
        question: '如何保证服务的可用性？',
        answer:
          '我们致力于提供高可用的MCP服务，并设有服务等级协议（SLA）。具体服务的SLA请参考其文档。',
      },
      {
        question: '我可以创建自己的MCP服务吗？',
        answer:
          '是的，我们提供了完善的开发者工具和文档，帮助您轻松创建和发布自己的MCP服务。',
      },
    ];

    return {
      hero: {
        title: 'AIbase MCP 服务中心',
        subtitle: '探索、集成并管理您的微组件化服务。',
      },
      infoCards: [
        {
          title: '什么是MCP？',
          description: '了解MCP架构的核心概念。',
          linkUrl: '/mcp/docs/what-is-mcp',
        },
        {
          title: '快速入门指南',
          description: '几分钟内集成您的第一个MCP服务。',
          linkUrl: '/mcp/docs/quickstart',
        },
        {
          title: '服务市场',
          description: '浏览可用的MCP服务。',
          linkUrl: '/mcp/services',
        },
        {
          title: '开发者文档',
          description: '深入了解MCP SDK和API。',
          linkUrl: '/mcp/docs/developer',
          isComingSoon: true,
    ...
    const additionalServices = [
      {
        ...
        description: 'Handles all kinds of notifications via MCP.',
        externalServiceUrl: '#',
        languageOrTech: 'Java',
        userCountOrStars: 1200,
        rating: '4.7',
        isVerified: true,
        imageUrl: 'https://via.placeholder.com/100x70?text=NotifyMCP',
      },
      {
        id: 'mcp-s2',
        name: 'Data Storage MCP',
        description: 'Scalable and reliable data storage MCP.',
        externalServiceUrl: '#',
        languageOrTech: 'Go',
        userCountOrStars: '2.1K',
        rating: '4.9',
        isVerified: true,
        imageUrl: 'https://via.placeholder.com/100x70?text=StoreMCP',
      },
      {
        id: 'mcp-s3',
        name: 'Logging MCP Framework',
        description: 'Centralized logging for distributed MCPs.',
        externalServiceUrl: '#',
        languageOrTech: 'Python',
        userCountOrStars: 750,
        rating: '4.3',
        isVerified: false,
        imageUrl: 'https://via.placeholder.com/100x70?text=LogMCP',
      },
      {
        id: 'mcp-s4',
        name: 'Auth MCP Deluxe',
        description: 'Deluxe authentication and authorization MCP.',
        externalServiceUrl: '#',
        languageOrTech: 'Ruby',
        userCountOrStars: '500',
        rating: '4.6',
        isVerified: true,
        imageUrl: 'https://via.placeholder.com/100x70?text=AuthDlx',
      },
    ];
    ...
    const filteredServices = searchTerm
      ? allServices.filter(
          (service) =>
            ...
            (service.languageOrTech &&
              service.languageOrTech.toLowerCase().includes(searchTerm)),
        )
  ...
  getMcpServicesByTag(
    ...
    const tagName = tagSlug
      .replace(/-/g, ' ')
      .replace(/\b\w/g, (l) => l.toUpperCase()); // Convert slug to readable name

    // 2. Filter by language/tech (lang query param)
    if (lang) {
      services = services.filter(
        (s) => s.languageOrTech?.toLowerCase() === lang.toLowerCase(),
      );
    }
    ...
    if (verified === 'true') {
      ...
      services = services.filter(
        (s) => s.isVerified === false || s.isVerified === undefined,
      );
    }
    ...
    if (sort === 'popularity') {
      // Placeholder: sort by userCount/Stars desc
      services.sort(
        (a, b) =>
          (parseFloat(
            String(b.userCountOrStars)
              .replace('K', '000')
              .replace('M', '000000'),
          ) || 0) -
          (parseFloat(
            String(a.userCountOrStars)
              .replace('K', '000')
              .replace('M', '000000'),
          ) || 0),
      );
      ...
      services.sort(
        (a, b) =>
          (parseFloat(String(b.rating)) || 0) -
          (parseFloat(String(a.rating)) || 0),
      );
    } else if (sort === 'newest') {
      // Placeholder: sort by ID desc for mock
    ...
    const availableLanguages = [
      ...new Set(services.map((s) => s.languageOrTech).filter(Boolean)),
    ].sort();
